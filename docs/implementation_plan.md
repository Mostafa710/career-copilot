...
graph TD
    ...
    API <--> Sources[Multi-Source Radar: JSearch v2 + Wuzzuf + Tavily]
...
* **Search & Intelligence:** Multi-source MENA & Global engine (RapidAPI JSearch v2 for LinkedIn/Indeed, Wuzzuf scraper, and Tavily live search fallback) with auto-cached company intelligence.
...
* **Immutable CV Version History:** Maintains rolling 4-version history with pinned protection for application-referenced snapshots.
...
* **Concurrent Ingestion ($0 Cost):**
  ...
  * *RapidAPI JSearch Client (v2):* Queries live **LinkedIn** and **Indeed** listings with normalized dictionary parsing.
* **Universal Dynamic Backfill:** Tavily live web search queries `linkedin.com` and `wuzzuf.net` if fewer than 15 jobs are collected, guaranteeing 15–20 distinct opportunities.
...
* **Fact & Anti-Hallucination Critic Node:** Validates tailored bullets against original CV. Rejects unverified skills, degrees, or companies (up to 3 total attempts) with multi-stage fallback parsing and strict 422 handling.
* **ATS Gap Delta:** Computes Before vs. After ATS match score.
* **Document Exports:** One-click downloads for Word (`.docx`) CV, Word (`.docx`) Cover Letter, and Plain Text (`.txt`) Email in Studio and Mini-CRM.

### Module 04: 6-Stage Mini-CRM Kanban Pipeline
* **Stages:** Saved, Tailored, Applied, Interviewing, Offered, Rejected.
* **Opportunity Drawer:** View tailored assets, download DOCX and TXT files, update application stage, or re-run tailoring with fresh parameters.

### Module 05: Stateful Mock Interview Simulator
* **Modes:**
  * *General:* Dynamic opening question tailored to domain and CV skills.
  * *Technical:* Dynamic opening question and technical depth tradeoffs strictly against selected Mini-CRM opportunities + active CV + company intelligence. Locked until opportunity selected.
  * *Behavioral:* Evaluates candidate responses against the STAR framework in the context of target Mini-CRM job + active CV + company culture values. Locked until opportunity selected.
* **State Machine:** Multi-turn conversation tracking with immediate per-turn micro-feedback.
* **Scorecard & Session Management:** Overall score (0–100), STAR rubric assessment, technical depth, strengths, areas for improvement, hiring recommendations, and dedicated Exit Interview button.
...
* **Session Lifecycle:** Interactive End Chat / New Session reset controls.
...
jobs (id, external_id TEXT, content_hash, source, title VARCHAR(1024), company VARCHAR(1024), location VARCHAR(1024), salary_min, salary_max, redirect_url TEXT, description TEXT, company_insights, embedding vector(384))
